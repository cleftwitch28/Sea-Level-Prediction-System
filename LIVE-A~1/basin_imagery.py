...
def apply_inundation(img, slr_mm, storm_risk='high', scale='city'):
    ...
    """
    # No rise (or a fall) vs the baseline → NO overlay at all. Without
    # this, the minimum-reach floor below painted a phantom 5 px flood
    # band even at slr_mm=0, so historical years (2015-2024) looked MORE
    # flooded than the 2025 baseline. The floor exists only to keep small
    # POSITIVE rises visible.
    if slr_mm <= 0:
        return img

...
def apply_inundation(img, slr_mm, storm_risk='high', scale='city'):
    ...
    raw_perm_reach = slr_mm / 12.0 * reach_factor
    ...
    min_dim = min(H, W)
    perm_reach = float(np.clip(raw_perm_reach, 3.0, 0.05 * min_dim))
    surge_mult = {'low': 1.8, 'medium': 2.6, 'high': 3.6, 'extreme': 4.8}.get(storm_risk, 3.0)
    surge_reach = float(np.clip(perm_reach * surge_mult, perm_reach, 0.15 * min_dim))
    ...
    perm_alpha = _gaussian_blur(perm_alpha, 1.6) ** 0.9
    surge_only = _gaussian_blur(surge_only, 2.6) ** 0.9

    perm_alpha[no_data] = 0.0
    surge_only[no_data] = 0.0

    CYAN = np.array([0.12, 0.55, 0.86], dtype=np.float32)
